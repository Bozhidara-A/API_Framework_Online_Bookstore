Feature: Books
  ...
  Scenario Outline: Retrieve a book by valid ID
    Given a book with ID <id> exists
    When I retrieve the book with ID: <id>
    Then the response status code should be 200
  ...
  Scenario Outline: Retrieve a not existing book
    Given a book with ID <id> does not exist
    When I retrieve the book with ID: <id>
  ...
  Scenario Outline: Retrieve a book by invalid ID
    When I retrieve the book with ID: <id>
  ...
  Scenario Outline: Update a book with valid data
    Given a book with ID <id> exists
    And a book with ID <new_id> does not exist
    ...
    Examples:
      ...
      | 100 | 400    | Updated Book | Book Description | 100        | Update  | 2025-04-03T00:00:00 |
  ...
  Scenario Outline: Update a book with invalid data
    ...
    Examples:
      ...
      | 12345678910 | 100         | Updated book | Long ID Book | 100        | test    | 2025-04-03T00:00:00 | One or more validation errors occurred. | "The value '12345678910' is not valid."                                                                         | "errors.id"     |
      | 100         | 12345678910 | Updated book | Long New ID  | 100        | test    | 2025-04-03T00:00:00 | One or more validation errors occurred. | "The JSON value could not be converted to System.Int32. Path: $.id \| LineNumber: 0 \| BytePositionInLine: 17." | "errors.'$.id'" |

  Scenario Outline: Update not existing book
    Given a book with ID <id> does not exist
    And a book with ID <new_id> does not exist
    When I update the book with ID: <id> with New ID: <new_id>, Title: <title>, Description: <description>, Page Count: <page_count>, Excerpt: <excerpt>, Publish Date: <publish_date>
    Then the response status code should be 404
    Examples:
      | id  | new_id | title        | description      | page_count | excerpt | publish_date        |
      | 500 | 400    | Updated Book | Book Description | 100        | Update  | 2025-04-03T00:00:00 |

  Scenario Outline: Update a book ID to already existing ID
    Given a book with ID <id> exists
  ...
  Scenario Outline: Delete a book with valid ID
    Given a book with ID <id> exists
  ...
  Scenario Outline: Delete a not existing book
    Given a book with ID <id> does not exist
    When I delete the book with ID: <id>
    Then the response status code should be 404
    Examples:
